...
type SceneState = Record<string, string>
type PendingField = { key: string; val: string }
...
// Keywords matched as substrings (case-insensitive). First match wins.
// Falls back to default purple.

const MOOD_COLORS: { words: string[]; hsl: [number, number, number] }[] = [
...
function getMoodHsl(mood: string): [number, number, number] {
...
function fieldLabel(key: string) { return key.charAt(0).toUpperCase() + key.slice(1) }
function escAttr(str: string)    { return str.replace(/&/g, '&amp;').replace(/"/g, '&quot;') }
function escHtml(str: string)    { return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') }

// ─── HTML builders ────────────────────────────────────────────────────────────

function buildViewHTML(state: SceneState): string {
...
function buildEditHTML(state: SceneState, pendingFields: PendingField[]): string {
...
function buildWidget(state: SceneState, editing: boolean, collapsed: boolean, pendingFields: PendingField[]): string {
  if (collapsed) {
    ...
    <div class="st-card">
      ...
      <div class="st-body">
        ${!hasState
          ...
          : editing ? buildEditHTML(state, pendingFields) : buildViewHTML(state)}
      </div>
    </div>
  `
}
...
function applyMoodTheme(widgetRoot: HTMLElement, moodValue: string) {
  const [h, s, l] = getMoodHsl(moodValue)
  const header = widgetRoot.querySelector<HTMLElement>('#st-header')
  const card   = widgetRoot.querySelector<HTMLElement>('.st-card')
  if (header) {
...
const STYLES = `
  ...
  .st-card {
    width: 300px; font-family: var(--lumiverse-font, system-ui, sans-serif);
    color: var(--lumiverse-text); background: rgba(18,14,38,0.72);
  ...
  .st-header {
    ...
    padding: 9px 12px 8px; border-bottom: 1px solid rgba(255,255,255,0.07);
    background: rgba(255,255,255,0.03); transition: background 1.2s ease, border-color 1.2s ease;
  }
  .st-title { font-size: 10px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(255,255,255,0.35); }
  .st-header-actions { display: flex; gap: 5px; align-items: center; }
  .st-btn { font-family: inherit; font-size: 10.5px; font-weight: 600; padding: 3px 9px; border-radius: 20px; cursor: pointer; border: 1px solid transparent; line-height: 1.5; transition: opacity 0.12s, background 0.12s; }
  .st-ghost { background: rgba(255,255,255,0.07); border-color: rgba(255,255,255,0.12); color: rgba(255,255,255,0.55); }
  ...
  .st-hero-row { display: flex; flex-direction: column; gap: 1px; padding: 7px 0; border-bottom: 1px solid rgba(255,255,255,0.055); }
  .st-hero-row:last-of-type { border-bottom: none; }
  .st-hero-label { font-size: 9.5px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: rgba(255,255,255,0.3); }
  .st-hero-value { font-size: 13.5px; font-weight: 500; color: rgba(255,255,255,0.92); line-height: 1.35; white-space: normal; word-break: break-word; }
  .st-chip-row { display: flex; flex-wrap: wrap; gap: 5px; }
  .st-chip-row--top { padding-bottom: 10px; margin-bottom: 4px; border-bottom: 1px solid rgba(255,255,255,0.055); }
  .st-chip { display: flex; flex-direction: column; gap: 1px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.09); border-radius: 8px; padding: 4px 8px 5px; flex: 1; min-width: 60px; }
  .st-chip-label { font-size: 8.5px; font-weight: 700; letter-spacing: 0.09em; text-transform: uppercase; color: rgba(255,255,255,0.28); }
  .st-chip-value { font-size: 11.5px; font-weight: 500; color: rgba(255,255,255,0.82); white-space: normal; word-break: break-word; line-height: 1.3; }
  .st-edit-row { display: flex; align-items: center; gap: 6px; padding: 4px 0; border-bottom: 1px solid rgba(255,255,255,0.045); }
  .st-edit-row:last-of-type { border-bottom: none; }
  .st-edit-label { min-width: 64px; font-size: 9.5px; font-weight: 700; letter-spacing: 0.09em; text-transform: uppercase; color: rgba(255,255,255,0.28); flex-shrink: 0; }
  .st-input { flex: 1; padding: 4px 8px; background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.12); border-radius: 7px; color: rgba(255,255,255,0.9); font-size: 11.5px; font-family: inherit; outline: none; min-width: 0; }
  .st-key-input { max-width: 90px; flex: 0 0 90px; }
  .st-input:focus { border-color: var(--lumiverse-accent, #7c4dff); }
  .st-del-btn { flex-shrink: 0; background: none; border: none; color: rgba(255,255,255,0.2); font-size: 15px; cursor: pointer; padding: 0 2px; line-height: 1; transition: color 0.12s; }
  .st-del-btn:hover { color: rgba(255,100,100,0.8); }
  .st-add-field-btn { display: flex; align-items: center; gap: 5px; margin-top: 8px; padding: 5px 10px; background: rgba(255,255,255,0.04); border: 1px dashed rgba(255,255,255,0.15); border-radius: 8px; color: rgba(255,255,255,0.35); font-family: inherit; font-size: 10.5px; font-weight: 600; cursor: pointer; width: 100%; justify-content: center; transition: background 0.12s, color 0.12s, border-color 0.12s; }
  .st-add-field-btn:hover { background: rgba(255,255,255,0.08); color: rgba(255,255,255,0.7); border-color: rgba(255,255,255,0.3); }
  .st-empty { font-size: 11.5px; color: rgba(255,255,255,0.3); font-style: italic; padding: 6px 0; margin: 0; }
  .st-pill { display: flex; align-items: center; gap: 6px; padding: 8px 12px 8px 10px; background: rgba(18,14,38,0.82); border: 1px solid rgba(255,255,255,0.09); border-radius: 999px; color: rgba(255,255,255,0.6); cursor: pointer; font-family: inherit; font-size: 11px; font-weight: 600; letter-spacing: 0.06em; box-shadow: 0 4px 18px rgba(0,0,0,0.5); transition: background 0.15s, color 0.15s, box-shadow 0.4s, border-color 0.4s; white-space: nowrap; }
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  let pendingFields:  PendingField[] = []
  ...
  function repaint() {
    ...
    if (!isCollapsed) applyMoodTheme(widgetRoot, sceneState['mood'] ?? '')

    widgetRoot.querySelector('#st-expand')?.addEventListener('click', () => { isCollapsed = false; repaint() })
    widgetRoot.querySelector('#st-collapse')?.addEventListener('click', () => {
      isCollapsed = true; isEditing = false; pendingFields = []; repaint()
    })
    widgetRoot.querySelector('#st-edit')?.addEventListener('click', () => { isEditing = true; repaint() })
    widgetRoot.querySelector('#st-cancel')?.addEventListener('click', () => {
      isEditing = false; pendingFields = []; pendingDeletions = []; repaint()
    })

    widgetRoot.querySelector('#st-save')?.addEventListener('click', () => {
      const patch: SceneState = {}

      widgetRoot.querySelectorAll<HTMLInputElement>('.st-input[data-key]').forEach(input => {
        if (input.dataset.key) patch[input.dataset.key] = input.value.trim() || sceneState[input.dataset.key]
      })

      widgetRoot.querySelectorAll<HTMLInputElement>('.st-key-input').forEach((keyInput, i) => {
        const valInput = widgetRoot.querySelector<HTMLInputElement>(`[data-new-val-index="${i}"]`)
      ...
      for (const k of pendingDeletions) delete sceneState[k]
      pendingDeletions = []
      isEditing = false
      pendingFields = []
      repaint()
    ...
    widgetRoot.querySelectorAll<HTMLElement>('[data-del-key]').forEach(btn => {
    ...
    widgetRoot.querySelectorAll<HTMLElement>('[data-del-pending]').forEach(btn => {
      btn.addEventListener('click', () => {
  ...
  function flashCard() {
    const el = widgetRoot.querySelector<HTMLElement>('.st-card')
    if (!el) return
    el.classList.remove('st-flash'); void el.offsetWidth; el.classList.add('st-flash')
  }

  function flashPill() {
    const el = widgetRoot.querySelector<HTMLElement>('.st-pill')
    if (!el) return
    el.classList.remove('st-flash'); void el.offsetWidth; el.classList.add('st-flash')
  }
